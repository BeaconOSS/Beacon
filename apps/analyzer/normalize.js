const FINDING_TYPES = new Set(["error", "testFail", "warn", "warning", "testWarn", "recommendation"]);
...
export function normalize({ cli, mcr, mctoolsVersion }) {
	...
	const features = items
		.filter((it) => it && it.type === "featureAggregate" && typeof it.message === "string" && it.message.length > 0)
		.map((it) => ({
			generatorId: str(it.generatorId),
	...
	const decision = counts.errors > 0 ? "fail" : counts.warnings > 0 ? "warn" : "pass";
