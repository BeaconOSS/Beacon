...
const MAX_ARCHIVE_BYTES = Number(
  process.env.MAX_ARCHIVE_BYTES || 64 * 1024 * 1024,
);
const MAX_UNCOMPRESSED_BYTES = Number(
  process.env.MAX_UNCOMPRESSED_BYTES || 512 * 1024 * 1024,
);
...
const MCT_BIN =
  process.env.MCT_BIN ||
  path.join(__dirname, "node_modules", ".bin", isWin ? "mct.cmd" : "mct");
    ...
    execFile(
      ...
      (error, stdout) => {
        if (error && error.killed) {
          reject(
            Object.assign(new Error("validation timed out"), { code: 504 }),
          );
          return;
        }
        resolve(stdout || "");
...
server.listen(PORT, () => {
  console.log(
    `mctools-analyzer listening on :${PORT} (mct ${mctoolsVersion()})`,
  );
});
