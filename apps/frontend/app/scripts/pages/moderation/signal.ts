...
export function buildDecisionSignal(review: PendingReview | null): DecisionSignal {
	...
	if (!analysis || analysis.status === "pending") {
		...
		validationStatus = decision === "fail" ? "fail" : decision === "warn" ? "warn" : "pass";
		const counts = analysis.report.counts;
		checks.push({
			label: "MCTools validation",
			status: validationStatus,
			detail: decision === "pass" ? "No errors or warnings" : `${plural(counts.errors, "error")}, ${plural(counts.warnings, "warning")}`,
		});
	} else {
		checks.push({
			label: "MCTools validation",
	...
	if (diff?.files_truncated) {
		...
		checks.push({
			label: "Pack contents",
			status: "pass",
			detail: plural(diff.added + diff.removed + diff.modified, "file change") + " indexed",
		});
	}
	...
	if (review) {
		...
		} else if (summary && title && summary.toLowerCase() === title.toLowerCase()) {
			smells.push({
				label: "Summary",
				status: "warn",
		...
		const insecureLinks = Object.values(review.links).filter((url) => url.trim().toLowerCase().startsWith("http://"));
	...
	const overall: SignalStatus =
		validationStatus === "pending" ? "pending" : checks.reduce<SignalStatus>((worst, check) => (SIGNAL_RANK[check.status] > SIGNAL_RANK[worst] ? check.status : worst), "pass");
