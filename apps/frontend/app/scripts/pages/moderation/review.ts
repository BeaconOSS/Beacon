...
export function useModerationReview() {
  ...
  } = useProjectPendingReview(slug);
  const {
    submitting: reviewSubmitting,
    error: reviewError,
    review,
  } = useProjectReview(slug);
  const {
    notes: moderatorNotes,
    submitting: noteSubmitting,
    load: loadModeratorNotes,
    add: addModeratorNote,
  } = useModeratorNotes(slug);
  ...
  const queueIndex = computed(() =>
    queueProjects.value.findIndex((p) => p.slug === slug),
  );
  const queuePosition = computed(() =>
    queueIndex.value >= 0 ? queueIndex.value + 1 : null,
  );
  const queueTotal = computed(() => queueProjects.value.length);
  const queuePrev = computed(() =>
    queueIndex.value > 0
      ? (queueProjects.value[queueIndex.value - 1] ?? null)
      : null,
  );
  const queueNext = computed(() =>
    queueIndex.value >= 0 && queueIndex.value < queueProjects.value.length - 1
      ? (queueProjects.value[queueIndex.value + 1] ?? null)
      : null,
  );
    ...
    try {
      if (kind === "text") {
        ...
        const [oldUrl, newUrl] = await Promise.all([
          wantOld
            ? fetchBlobUrl(oldVer as string, path)
            : Promise.resolve(null),
          wantNew
            ? fetchBlobUrl(newVer as string, path)
            : Promise.resolve(null),
        ]);
  ...
  const decisionSignal = computed(() =>
    buildDecisionSignal(pendingReview.value),
  );
