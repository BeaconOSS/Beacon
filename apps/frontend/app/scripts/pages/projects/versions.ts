...
export function useVersions(slug: string) {
	...
	async function load() {
		...
		pending.value = true;
		try {
			const data = await api<{ versions: Version[] }>(`/projects/${slug}/versions`);
	...
	function downloadUrl(version: Version): string {
		return `${config.public.apiBase}/projects/${slug}/versions/${encodeURIComponent(version.version_number)}/download`;
	}

	async function remove(version: Version): Promise<boolean> {
		error.value = "";
		try {
			await api(`/projects/${slug}/versions/${encodeURIComponent(version.version_number)}`, { method: "DELETE" });
