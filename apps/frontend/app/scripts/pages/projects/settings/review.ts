...
export function useSettingsReview(ctx: SettingsReviewContext) {
	...
	const changelogDirty = computed(() => {
		...
		return changelog.value.trim() !== (ctx.project.value.pending_changelog ?? "");
	});

	const isPublished = computed(() => ctx.project.value?.is_published === true);
	const hasPendingChanges = computed(() => ctx.project.value?.has_pending_changes === true);
	...
	const pendingChanges = computed(() => {
		...
		const add = (label: string, before: string, after: string, long = false) => {
			if (before.trim() !== after.trim()) rows.push({ label, before, after, long });
		};
		...
		add("Tags", published.categories.join(", "), p.categories.map((c) => c.name).join(", "));
		return rows;
	});
	...
	async function submitForReview(): Promise<boolean> {
		...
		try {
			const result = await api<{ status: ProjectStatus }>(`/projects/${ctx.slug}/submit`, { method: "POST", body: { changelog: changelog.value.trim() } });
	...
	async function withdrawFromReview(): Promise<boolean> {
		...
		submitting.value = true;
		try {
			const result = await api<{ status: ProjectStatus }>(`/projects/${ctx.slug}/withdraw`, { method: "POST" });
			if (ctx.project.value) ctx.project.value.status = result.status;
			await ctx.load();
			return true;
		} catch (err) {
