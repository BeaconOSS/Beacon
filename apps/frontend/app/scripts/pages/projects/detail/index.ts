...
export function useProjectDetail() {
	...
	const { project, error, pending, load: loadProject } = useProject(slug.value, previewPending.value);
	const { versions, load: loadVersions, downloadUrl } = useVersions(slug.value);
	const { images, load: loadGallery } = useGallery(slug.value);
	const { heartPending, savePending, toggleHeart, toggleSave } = useProjectInteractions(slug.value, project);

	async function load() {
		await Promise.all([loadProject(), loadVersions(), loadGallery()]);
	}

	const isOwner = computed(() => !!user.value && !!project.value && user.value.username === project.value.owner);
	...
	const typeStyle = computed<TypeStyle>(() => TYPE_STYLES[project.value?.project_type ?? "addon"] ?? TYPE_STYLES.addon!);
	...
	const downloadableVersions = computed(() => versions.value.filter((v) => v.file));

	const latestVersion = computed<Version | null>(() => downloadableVersions.value.find((v) => v.channel === VERSION_CHANNEL.RELEASE) ?? downloadableVersions.value[0] ?? null);

	const changelogEntries = computed(() => versions.value.filter((v) => v.changelog && v.changelog.trim().length > 0));
	...
	async function handleSave() {
		...
		try {
			await toggleSave();
			toast.success(project.value?.viewer_saved ? "Saved to your library." : "Removed from your saved projects.");
		} catch (err) {
			toast.error((err as Error).message);
		}
	}
	...
	function submitReport() {
		...
		reportReason.value = "";
		toast.info("Reporting isn't available yet - thanks, we've noted your interest.");
	}

	const isPreview = computed(() => project.value?.preview === true);

	const showOwnerPending = computed(() => !isPreview.value && isOwner.value && project.value?.has_pending_changes === true);
