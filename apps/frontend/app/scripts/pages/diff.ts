...
export function looksLikePlaceholder(text: string): boolean {
	...
	for (let len = 1; len <= 8 && len < compact.length; len++) {
		...
		if (unit.repeat(Math.ceil(compact.length / len)).slice(0, compact.length) === compact) {
			return true;
		}
	}
...
const TEXT_EXTENSIONS = new Set(["json", "geo", "material", "lang", "mcfunction", "txt", "properties", "js", "ts", "html", "css", "md"]);
