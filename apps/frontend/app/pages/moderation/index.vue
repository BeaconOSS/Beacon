	...
	<div class="page-canvas min-h-screen">
		<div class="mx-auto max-w-5xl px-6 py-10">
			<div class="mb-8 flex items-center gap-3">
				<div class="bg-primary/15 text-primary flex size-11 items-center justify-center rounded-2xl">
					<ShieldCheck class="size-6" />
				</div>
				<div>
					<h1 class="display-heading text-2xl sm:text-3xl">Review queue</h1>
					<p class="text-muted-foreground text-sm">Projects submitted by creators awaiting moderation.</p>
				</div>
			</div>
			...
			<div v-else-if="error" class="border-border/60 rounded-xl border p-10 text-center">
				<p class="text-muted-foreground">{{ error }}</p>
				<NuxtLink v-if="!isModerator" to="/" class="text-primary mt-3 inline-block text-sm hover:underline"> Back to home </NuxtLink>
			</div>

			<div v-else-if="!projects.length" class="border-border/60 rounded-2xl border border-dashed p-12 text-center">
				<ShieldCheck class="text-muted-foreground/50 mx-auto mb-3 size-10" />
				<p class="text-foreground font-medium">The queue is clear</p>
				<p class="text-muted-foreground mt-1 text-sm">There are no projects awaiting review right now.</p>
			</div>
			...
			<ul v-else class="space-y-3">
				<li v-for="item in projects" :key="item.id">
					...
					>
						<div v-if="iconUrl(item)" class="size-14 shrink-0 overflow-hidden rounded-xl ring-1 ring-white/10">
							<img :src="iconUrl(item)!" :alt="item.title" class="size-full object-cover" />
						</div>
						<div v-else class="flex size-14 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ring-1 ring-white/10" :class="typeStyle(item.project_type).gradient">
							<component :is="typeStyle(item.project_type).icon" class="size-7" />
						</div>
						...
						<div class="min-w-0 flex-1">
							...
							<div class="text-muted-foreground mt-2 flex flex-wrap items-center gap-4 text-xs">
