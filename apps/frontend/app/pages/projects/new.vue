...
<template>
	<div class="page-canvas">
		<div class="mx-auto max-w-2xl px-6 py-12">
			<NuxtLink to="/projects" class="text-muted-foreground hover:text-foreground mb-6 inline-flex items-center gap-1.5 text-sm transition-colors">
			...
			<header class="mb-8">
				...
				<p class="text-muted-foreground">Start with the basics - you can flesh out the rest from the project settings afterwards.</p>
			</header>

			<form class="card-glass space-y-8 rounded-2xl p-6 md:p-8" @submit.prevent="submit">
				<div class="space-y-3">
					<Label>Type</Label>
					<div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
						<button
							...
							:class="projectType === type.value ? 'border-primary/70 bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/60 hover:text-foreground'"
							@click="projectType = type.value"
						>
							<span class="icon-chip size-10 rounded-xl" :style="{ background: typeStyle(type.value).gradient }">
								<component :is="typeStyle(type.value).icon" class="size-5" :stroke-width="2.25" />
							</span>
							{{ type.label }}
						</button>
					</div>
				...
				<div class="space-y-2">
					...
					<Input id="title" v-model="title" name="title" placeholder="Best Project Ever" />
				</div>
				...
				<div class="space-y-3">
					...
					<div class="grid gap-3 sm:grid-cols-3">
						<button
							...
							:class="visibility === option.value ? 'border-primary/70 bg-primary/10' : 'border-border hover:border-primary/60'"
							@click="visibility = option.value"
						>
							<span class="inline-flex items-center gap-2 text-sm font-semibold" :class="visibility === option.value ? 'text-primary' : 'text-foreground'">
				...
				<div class="space-y-2">
					...
					<Textarea id="summary" v-model="summary" name="summary" :rows="2" placeholder="A sentence or two describing your project." />
				</div>
