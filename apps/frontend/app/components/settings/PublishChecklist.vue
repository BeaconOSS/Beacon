  ...
  <section class="card-glass rounded-2xl p-6">
    ...
    <div class="mt-6 space-y-2 border-t pt-5">
      ...
      <p class="text-muted-foreground text-xs">
        Tell moderators what changed in this submission. Shown to the review
        team alongside a diff of your edits.
      </p>
      <MarkdownEditor
        id="changelog-note"
        v-model="changelog"
        :actions="NOTE_MARKDOWN_ACTIONS"
        :rows="4"
      ...
      >
        <span v-if="changelogError" class="text-destructive text-xs">{{
          changelogError
        }}</span>
        <Button
          variant="outline"
    ...
    >
      <p class="text-muted-foreground text-xs">
        Your project stays private until a moderator reviews and approves it.
        Approved projects re-enter review when key details change.
      </p>
      <div
      ...
      <div
        v-else-if="status === PROJECT_STATUS.APPROVED && !hasPendingChanges"
