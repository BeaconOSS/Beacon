...
<template>
	<section class="space-y-6">
		<div class="card-glass space-y-5 rounded-2xl p-6">
			<div>
				...
				<p class="text-muted-foreground text-sm leading-relaxed">
					Every release of your project is a version. Upload the downloadable file, give it a version number, and add a changelog so people know what changed.
				</p>
			</div>
			...
			<div class="grid gap-4 sm:grid-cols-2">
				<div class="space-y-2">
					...
					<Input id="version-number" v-model="versionNumber" placeholder="1.0.0" />
				</div>
				<div class="space-y-2">
					...
					<Select id="version-channel" v-model="channel">
						...
						<SelectContent>
							<SelectItem v-for="option in VERSION_CHANNELS" :key="option.value" :value="option.value">
								{{ option.label }}
							</SelectItem>
						</SelectContent>
					</Select>
			...
			<div class="space-y-2">
				...
				<Input id="version-name" v-model="name" placeholder="e.g. Winter Update" />
			</div>

			<div class="space-y-2">
				<Label for="version-changelog">Changelog (optional)</Label>
				<Textarea id="version-changelog" v-model="changelog" rows="4" placeholder="What changed in this version?" />
			</div>
			...
			<div class="space-y-2">
				...
				/>
				<p v-if="file" class="text-muted-foreground text-xs">{{ file.name }} · {{ formatBytes(file.size) }}</p>
			</div>
		...
		<div class="card-glass rounded-2xl p-6">
			...
			<p v-if="!versions.length" class="text-muted-foreground border-border/60 rounded-xl border border-dashed p-8 text-center text-sm">No versions yet. Upload your first version above.</p>

			<ul v-else class="space-y-3">
				<li v-for="version in versions" :key="version.id" class="border-border/60 bg-muted/20 rounded-xl border p-4">
					<div class="flex flex-wrap items-start justify-between gap-3">
						<div class="min-w-0">
							<div class="flex flex-wrap items-center gap-2">
								...
								<span class="rounded-full px-2 py-0.5 text-[11px] font-semibold" :class="CHANNEL_STYLES[version.channel] ?? 'bg-muted text-muted-foreground'">
						...
						<div class="flex shrink-0 items-center gap-2">
							...
							<template v-if="confirmDeleteVersionId === version.id">
								...
								<Button variant="ghost" size="sm" @click="confirmDeleteVersionId = null"> Cancel </Button>
							</template>
							<Button v-else variant="ghost" size="icon" aria-label="Delete version" :disabled="locked" @click="confirmDeleteVersionId = version.id">
								<Trash2 class="text-destructive size-4" />
							</Button>
						</div>
					</div>

					<div v-if="version.changelog" class="text-muted-foreground mt-3 border-t pt-3 text-sm whitespace-pre-line">
						{{ version.changelog }}
					</div>
				</li>
			</ul>
