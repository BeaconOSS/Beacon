...
<template>
	<div class="border-border/60 bg-card/30 rounded-xl border p-6">
		<ul v-if="versions.length" class="divide-border/60 divide-y">
			<li v-for="v in versions" :key="v.id" class="flex flex-wrap items-center justify-between gap-3 py-3 first:pt-0">
				<div class="min-w-0">
				...
				<Button v-if="v.file" as-child variant="outline" size="sm" class="gap-2">
