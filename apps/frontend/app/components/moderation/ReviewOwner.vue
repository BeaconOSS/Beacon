...
<template>
	<div class="border-border/60 bg-card/40 rounded-2xl border p-5">
		<p class="text-muted-foreground mb-3 text-xs font-semibold tracking-wide uppercase">Creator</p>
		<div class="mb-3 flex items-center gap-2">
			<div class="bg-primary/15 text-primary flex size-9 items-center justify-center rounded-full">
				<User class="size-4" />
			</div>
			<div class="min-w-0">
				...
				<p class="text-muted-foreground text-xs">Joined {{ formatDate(owner.member_since) }}</p>
			</div>
		</div>
		<span v-if="trust" class="mb-3 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold" :class="trust.class">
