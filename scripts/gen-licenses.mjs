...
function collectFrontend() {
	...
	for (const name of names) {
		...
		const spdx = typeof meta.license === "string" ? meta.license : (meta.license?.type ?? (Array.isArray(meta.licenses) ? meta.licenses.map((l) => l.type).join(" OR ") : ""));
...
function collectBackend() {
	const raw = execFileSync("cargo", ["metadata", "--format-version", "1", "--manifest-path", join(BACKEND, "Cargo.toml")], { encoding: "utf8", maxBuffer: 64 * 1024 * 1024 });
	...
	const directNames = new Set(root.dependencies.filter((d) => d.kind == null).map((d) => d.name));
	const libraries = [];
